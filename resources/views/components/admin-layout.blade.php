    ...
    <div class="flex h-dvh overflow-hidden">
        ...
        <aside :class="open ? 'translate-x-0' : '-translate-x-full md:translate-x-0'"
            ...
            <div class="h-16 flex items-center px-6 border-b border-dark-border shrink-0">
                @php $adminProfile = App\Models\User::where('is_admin', true)->first()?->profile; @endphp
                <a href="{{ route('home') }}" class="font-heading text-2xl font-black gradient-text">{{ $adminProfile?->avatar_initials ?: 'JF' }}</a>
            ...
            <div class="px-4 py-4 border-b border-dark-border">
                <div class="flex items-center gap-3">
                    <div class="w-8 h-8 rounded-full shrink-0 overflow-hidden">
                        @if($adminProfile?->avatar_path)
                            <img src="{{ asset('storage/' . $adminProfile->avatar_path) }}" alt="Avatar" class="w-full h-full object-cover">
                        @else
                            <div class="w-full h-full bg-aurora flex items-center justify-center text-white text-xs font-bold">
                                {{ $adminProfile?->avatar_initials ?: strtoupper(substr(Auth::user()->name, 0, 2)) }}
                            </div>
                        @endif
                    </div>
                    <div class="min-w-0">
            ...
            <div class="px-3 py-4 border-t border-dark-border space-y-1 shrink-0">
                ...
                <a href="{{ route('admin.settings.index') }}"
                   class="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm font-semibold transition-all duration-150
                          {{ request()->routeIs('admin.settings.*') ? 'bg-brand-purple text-white' : 'text-slate-400 hover:text-slate-100 hover:bg-dark-elevated' }}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
