            ...
            <div class="grid lg:grid-cols-3 gap-8">
                ...
                <aside class="space-y-8">
                    ...
                    <div class="card-glass rounded-2xl p-5 space-y-3">
                        ...
                        <a href="tel:{{ preg_replace('/\s+/', '', $user->profile->phone) }}" class="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400 hover:text-brand-purple transition-colors">
                            <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4 shrink-0 text-brand-teal" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    @endif
                    {{-- Idiomas --}}
                    @if($user->languages->where('show_in_web', true)->isNotEmpty())
                    <div class="card-glass rounded-2xl p-5 space-y-3">
                        <h3 class="font-heading font-bold text-sm uppercase tracking-widest text-slate-500 dark:text-slate-400 mb-4">Idiomas</h3>
                        @foreach($user->languages->where('show_in_web', true) as $lang)
                        <div class="flex items-center justify-between">
                            <span class="text-sm font-semibold">{{ $lang->name }}</span>
                            <span class="badge badge-purple text-xs">{{ $lang->level }}</span>
                        </div>
                        @endforeach
                    </div>
                    @endif
